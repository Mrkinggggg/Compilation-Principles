...
void dfa_export_dot(const DFA* dfa, const char* filename) {
    ...
    for (uint8_t i = 0; i < dfa->state_count; i++) {
        ...
        for (uint8_t j = 0; j < dfa->states[i].item_count; j++) {
            const DFAItem* item = &dfa->states[i].items[j];
            fprintf(f, "%c -> ", item->left_symbol);
            for (uint8_t k = 0; k < item->right_len; k++) {
                if (k == item->dot) fprintf(f, "•");
                fprintf(f, "%c", item->right_symbols[k]);
            }
            if (item->dot == item->right_len) fprintf(f, "•");
            fprintf(f, "\n");
        }
        fprintf(f, "\"];\n");
    }
    ...
    // 输出所有转换
    for (uint16_t i = 0; i < dfa->transition_count; i++) {
